...
function smokeIdentity() {
  ...
  return {
    username: `commercial_${suffix}`.slice(0, 32),
    password: `Commercial-${suffix}-Password!aA1`
  };
}

test('desktop commercial runtime stays default-off, hidden and fail-closed', async ({ request }) => {
  const capabilitiesResponse = await request.get(`${worker}/api/capabilities`, {
    headers: { origin: 'http://127.0.0.1:4173' }
  });
  expect(capabilitiesResponse.status()).toBe(200);
  expect(capabilitiesResponse.headers()['access-control-allow-origin']).toBe('http://127.0.0.1:4173');
  expect(capabilitiesResponse.headers()['x-frame-options']).toBe('DENY');
  expect(capabilitiesResponse.headers()['x-content-type-options']).toBe('nosniff');
  expect(capabilitiesResponse.headers()['content-security-policy']).toContain("frame-ancestors 'none'");
  ...
  const rejectedOrigin = await request.get(`${worker}/api/capabilities`, {
  ...
  const registration = await request.post(`${worker}/api/auth/register`, {
    data: {
      ...
      displayName: 'Commercial Runtime Smoke',
