...
-- Верни metadata evidence через UNION ALL.`,
    solution: `CREATE TEMP TABLE contact_points(contact_id INTEGER PRIMARY KEY, channel TEXT NOT NULL CHECK (channel IN ('email','sms')), destination_digest TEXT NOT NULL UNIQUE, state TEXT NOT NULL CHECK (state IN ('active','revoked'))); CREATE TEMP VIEW active_contacts AS SELECT contact_id, channel, destination_digest FROM contact_points WHERE state = 'active'; SELECT 'column:' || name AS contract_item, CASE WHEN pk = 1 THEN 'primary-key' WHEN notnull = 1 THEN 'not-null' ELSE 'nullable' END AS contract_value FROM pragma_table_info('contact_points') WHERE name IN ('contact_id','channel','destination_digest','state') UNION ALL SELECT 'view:active_contacts', type FROM sqlite_temp_master WHERE type = 'view' AND name = 'active_contacts' ORDER BY contract_item;`,
    hints: [
