...
export const conceptInventory = {
  'sql-thinking': [concept(
    ...
    'Learner может словами описать одну строку результата и проверить SQL на минимальном наборе.',
    [
      ...
      misconception('order-is-default', 'Порядок строк возникает сам', 'Без ORDER BY база не обещает повторяемый порядок.', 'Добавь полный sort key с tie-breaker, если порядок входит в контракт.'),
  ...
  select: [concept(
    ...
    'Learner отличает выбор исходного поля от вычисления и объясняет тип/имя каждого результата.',
    [
      misconception('alias-speeds-query', 'Алиас ускоряет запрос', 'AS меняет имя результата, но не стоимость вычисления.', 'Объясни алиас как часть API результата, а производительность проверяй планом.'),
  ...
  filtering: [concept(
    ...
    'Learner предсказывает строки для TRUE/FALSE/UNKNOWN и ставит скобки вокруг смешанных AND/OR.',
    [
