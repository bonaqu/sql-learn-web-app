...
const bridgeNarratives: Record<string, BridgeNarrative> = {
  'sql-thinking->select': {
    ...
    limitation: 'Один контракт ещё не создаёт набор данных — нужно научиться явно выбирать и вычислять его столбцы.',
  ...
  'select->filtering': {
    carryForward: 'Продолжай контролировать форму результата и смысл каждого выражения в SELECT.',
    limitation: 'Правильные столбцы бесполезны, если в результат попали строки, которые не отвечают бизнес-условию.',
    newMentalModel: 'WHERE — проверяемый предикат допуска строки; несколько условий образуют булеву модель отбора.',
    evidencePrompt: 'Раздели условия на обязательные и альтернативные и проверь скобки на пограничных строках.'
  },
  'filtering->sorting': {
    carryForward: 'Сначала сформируй корректное множество строк с помощью явного предиката.',
