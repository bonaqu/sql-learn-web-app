...
function difference(next: number | undefined, previous: number | undefined) {
...
function recordTaskDelta(taskId: string, next: TaskStats, previous: TaskStats | undefined, nextProgress: Progress, previousProgress: Progress) {
  ...
  if (!task) return;
  const attempts = difference(next.attempts, previous?.attempts);
  const incorrect = difference(next.incorrect, previous?.incorrect);
  const independent = difference(next.independentPasses, previous?.independentPasses);
  const hints = difference(next.hintsUsed, previous?.hintsUsed);
  const solutionViews = difference(next.solutionViews, previous?.solutionViews);
  const newlyUnderstood = nextProgress.completed.includes(taskId) && !previousProgress.completed.includes(taskId);

  for (let index = 0; index < attempts; index += 1) {
    appendLearningEvent({
      type: 'attempted', taskId, moduleId: task.module,
  ...
  if (newlyUnderstood) {
    appendLearningEvent({ type: 'understood', taskId, moduleId: task.module, correct: true });
  }
  if (hints > 0) {
    appendLearningEvent({ type: 'remediation_started', taskId, moduleId: task.module, remediation: 'hint' });
  }
  if (solutionViews > 0) {
    appendLearningEvent({ type: 'remediation_started', taskId, moduleId: task.module, remediation: 'solution' });
  }
  if (independent > 0) {
    ...
    if ((previous?.incorrect || 0) > 0 || (previous?.hintsUsed || 0) > 0 || (previous?.solutionViews || 0) > 0) {
      appendLearningEvent({ type: 'remediation_completed', taskId, moduleId: task.module, remediation: 'retry', correct: true, independent: true });
    }
  }
}
...
export default function LearningAnalyticsAgent() {
  ...
  useEffect(() => {
    ...
    const started = state?.events.find(event => event.sessionId === sessionId && event.type === 'session_started');
  ...
  useEffect(() => {
    const onProgress = (event: Event) => {
      ...
      const keys = new Set([...Object.keys(previous.taskStats), ...Object.keys(next.taskStats)]);
