...
export default function ConceptCheckPanel({
  ...
  const [selections, setSelections] = useState<Record<string, number | null>>(() => Object.fromEntries(
    checks.map(check => [check.id, curriculum.answers[check.id]?.optionIndex ?? null])
  ));
  ...
  const submit = (checkId: string) => {
    ...
    const next = answerCurriculumCheck(curriculum, lesson.id, optionIndex, checkId);
    onProgress(next);
  };
  ...
  const runCounterexample = async (id: string, example: RunnableCounterexample) => {
    ...
    try {
      ...
      try {
        ...
        setCounterexampleResults(current => ({
          ...current,
          [id]: {
            wrong: wrong as SqlTable[],
            correct: correct as SqlTable[],
      ...
      setCounterexampleResults(current => ({
        ...
        [id]: {
          wrong: [],
          correct: [],
          message: `SQLite error: ${reason instanceof Error ? reason.message : String(reason)}`
        }
      }));
    } finally {
      setRunningId(null);
  ...
  return <section className="concept-check-panel" data-testid="concept-check-panel">
    <header className="concept-check-header">
      <div><small>Concept inventory</small><h2>{concept?.title || 'Проверяемая модель'}</h2><p>{concept?.mentalModel || lesson.subtitle}</p></div>
      <span className={progress.complete ? 'complete' : ''}><strong>{progress.completed}/{progress.total}</strong><small>обязательных checks</small></span>
    </header>
      ...
      {checks.map((check, checkIndex) => {
        ...
        return <article className={`concept-check-card ${answer?.correct ? 'correct' : answer ? 'incorrect' : ''}`} key={check.id} data-testid={`concept-check-${check.kind}`}>
          ...
          <fieldset>
            ...
            {check.options.map((option, optionIndex) => <label className={selected === optionIndex ? 'selected' : ''} key={`${check.id}-${optionIndex}`}>
              <input
                type="radio"
                name={check.id}
                checked={selected === optionIndex}
                onChange={() => setSelections(current => ({ ...current, [check.id]: optionIndex }))}
              />
              <span>{String.fromCharCode(65 + optionIndex)}</span><b>{option}</b>
            </label>)}
          </fieldset>
          <button className="concept-check-submit" onClick={() => submit(check.id)} disabled={selected === null || selected === undefined}>
            {answer ? <RefreshCw /> : <ShieldQuestion />}{answer ? 'Проверить ещё раз' : 'Проверить reasoning'}
          </button>
          {answer && <div className={`concept-option-feedback ${answer.correct ? 'success' : 'error'}`} role="status" aria-live="polite">
            {answer.correct ? <CheckCircle2 /> : <AlertTriangle />}
            <div><strong>{answer.correct ? 'Mental model подтверждён' : misconception ? `Заблуждение: ${misconception.label}` : 'Ответ требует remediation'}</strong><p>{check.optionFeedback[answer.optionIndex]}</p>{!answer.correct && <b>{misconception?.remediation || check.remediation}</b>}</div>
          </div>}
        </article>;
      })}
    </div>

    {concept?.misconceptions.some(item => item.counterexample) && <div className="counterexample-lab" data-testid="counterexample-lab">
      <div className="counterexample-lab-heading"><FlaskConical /><div><small>Executable counterexamples</small><h3>Сначала предскажи, потом сравни</h3><p>Оба запроса выполняются на одном локальном SQLite seed. Рабочие данные не отправляются.</p></div></div>
      {concept.misconceptions.filter(item => item.counterexample).map(item => {
        const example = item.counterexample!;
        const output = counterexampleResults[item.id];
        return <article key={item.id}>
          <header><div><strong>{item.label}</strong><p>{example.prediction}</p></div><button onClick={() => void runCounterexample(item.id, example)} disabled={runningId === item.id}><Play />{runningId === item.id ? 'Выполняю…' : 'Сравнить SQL'}</button></header>
          <div className="counterexample-code-grid"><div><small>Misconception SQL</small><pre><code>{example.wrongSql}</code></pre></div><div><small>Corrected SQL</small><pre><code>{example.correctSql}</code></pre></div></div>
          {output && <div className="counterexample-output" role="status"><div><small>Неверная модель</small><p>{resultSummary(output.wrong)}</p></div><div><small>Исправленная модель</small><p>{resultSummary(output.correct)}</p></div><b>{output.message}</b></div>}
        </article>;
      })}
    </div>}
  </section>;
