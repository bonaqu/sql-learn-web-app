...
export default function OnboardingPortal({ openRequest = 0 }: { openRequest?: number }) {
  ...
  const content = step === 'goal' ? <section className="onboarding-step" data-testid="onboarding-goal">
    ...
    <div className="onboarding-choice-grid goal-grid" role="radiogroup" aria-label="Цель обучения">{goalOptions.map((item, index) => <button
      key={item.id}
      role="radio"
      aria-checked={profile.goal === item.id}
      tabIndex={profile.goal === item.id || !profile.goal && index === 0 ? 0 : -1}
      className={profile.goal === item.id ? 'selected' : ''}
      onKeyDown={event => handleRadioArrow(event, goalOptions.map(option => option.id), selectGoal)}
    ...
    <div className="onboarding-minutes" role="radiogroup" aria-label="Минут в учебный день">{([15, 25, 40] as const).map(value => <button role="radio" aria-checked={profile.dailyMinutes === value} tabIndex={profile.dailyMinutes === value ? 0 : -1} className={profile.dailyMinutes === value ? 'selected' : ''} key={value} onKeyDown={event => handleRadioArrow(event, [15, 25, 40].map(String), selected => update({ dailyMinutes: Number(selected) as 15 | 25 | 40 }))} onClick={() => update({ dailyMinutes: value })}><Clock3 /><strong>{value}</strong><span>минут</span></button>)}</div>
    ...
    <div className="onboarding-pace" role="radiogroup" aria-label="Темп обучения">{paceOptions.map(item => <button key={item.id} role="radio" aria-checked={profile.pace === item.id} tabIndex={profile.pace === item.id ? 0 : -1} className={profile.pace === item.id ? 'selected' : ''} onKeyDown={event => handleRadioArrow(event, paceOptions.map(option => option.id), value => update({ pace: value }))} onClick={() => update({ pace: item.id })}><Gauge /><span><strong>{item.title}</strong><small>{item.detail}</small></span></button>)}</div>
