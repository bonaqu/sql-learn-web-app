...
export default function LessonMasteryPanel({
  ...
  const steps = [
    ...
    { id: 'check', title: 'Предсказать и объяснить', detail: mastery.checkCorrect ? `${mastery.checksCompleted}/${mastery.checksTotal} concept checks` : `нужно ${mastery.checksCompleted}/${mastery.checksTotal}`, done: mastery.checkCorrect, icon: <ShieldCheck /> },
    { id: 'practice', title: 'Применить самостоятельно', detail: mastery.applied ? `${mastery.independentTaskIds.length} independent SQL evidence` : 'без подсказки и эталона', done: mastery.applied, icon: <Code2 /> },
    { id: 'review', title: 'Сохранить навык', detail: mastery.retained ? `${mastery.reviewRepetitions} retrieval repetition` : mastery.reviewIntroduced ? 'карточка введена в расписание' : 'появится после evidence', done: mastery.retained, icon: <Repeat2 /> }
  ];
  ...
  return <>
    ...
    <section className="lesson-mastery-loop" data-testid="lesson-mastery-loop">
      ...
      {!mastery.mastered && <div className="lesson-mastery-next"><Circle /><div><strong>Следующий обязательный шаг</strong><p>{mastery.blocker}</p></div>{mastery.nextAction === 'practice' && nextPracticeTaskId && <button onClick={() => onOpenTask(nextPracticeTaskId)}><Code2 />Открыть independent practice</button>}</div>}
      {mastery.mastered && !mastery.retained && <div className="lesson-mastery-next review"><Repeat2 /><div><strong>Applied mastery получен</strong><p>Теперь не перечитывай урок. Дождись due-карточки и воспроизведи модель по памяти.</p></div><button onClick={onOpenReview}><Repeat2 />Открыть Review Deck</button></div>}
      {remediation && <div className="lesson-remediation" data-testid="lesson-remediation"><AlertTriangle /><div><small>Targeted remediation · {remediation.count} сигналов{remediation.conceptTitle ? ` · ${remediation.conceptTitle}` : ''}</small><strong>{remediation.title}</strong><p>{remediation.explanation}</p><b>{remediation.nextStep}</b></div>{remediation.taskId && <button onClick={() => onOpenTask(remediation.taskId!)}>Повторить задачу</button>}</div>}
    </section>
  </>;
}
