...
for (const boundary of ['assessment', 'learning-path', 'CurriculumPortal', 'SyllabusPortal', 'sqlite', 'ActivityChart', 'SqlEditor']) {
  if (!js.some(name => name.startsWith(`${boundary}-`) || name.includes(`-${boundary}-`))) {
    fail(`expected a separate ${boundary} chunk`);
  }
}

for (const heavy of ['assessment-', 'learning-path-', 'CurriculumPortal-', 'SyllabusPortal-', 'sqlite-', 'ActivityChart-', 'SqlEditor-']) {
