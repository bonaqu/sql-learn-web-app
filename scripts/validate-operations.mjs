...
try {
  ...
  execFileSync(process.execPath, ['scripts/verify-d1-backup.mjs', backup], {
    cwd: process.cwd(),
    stdio: 'pipe'
  });

  writeFileSync(backup, `${sql}\n-- tampered after manifest`);
  const tampered = spawnSync(process.execPath, ['scripts/verify-d1-backup.mjs', backup], {
    cwd: process.cwd(),
    encoding: 'utf8'
  });
  ...
  for (const guard of [
    'RESTORE_TO_NON_PRODUCTION',
    'Refusing to restore into the source/production database name',
    'rehearsal|restore|staging|test',
    'verify-d1-backup.mjs'
  ]) {
