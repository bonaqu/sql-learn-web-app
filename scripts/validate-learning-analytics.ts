...
import { emptyProgress } from '../src/lib/progress.ts';
...
assert(/REFERENCES\s+users\s*\(\s*user_id\s*\)\s+ON DELETE CASCADE/gi.test(migration), 'Analytics tables must cascade with account deletion');
...
assert(worker.includes("sharing !== 'coarse-opt-in'"), 'Snapshot writes must require explicit opt-in');
assert(worker.includes("DELETE FROM learning_analytics_snapshots"), 'Opt-out/delete must remove account snapshots');
assert(worker.includes('retained <= independent'), 'Impossible funnel states need validation');
...
console.log('Learning analytics validation passed: versioned local events, deterministic interventions, SQL-free coarse snapshots, explicit opt-in, k=5 suppression, export/delete and D1 cascade privacy contracts.');
