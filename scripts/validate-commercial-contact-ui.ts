...
const portalPath = new URL('../src/components/CommercialIdentityPortalV2.tsx', import.meta.url);
...
const portalPosition = main.indexOf('<CommercialIdentityPortalV2 />');
...
assert.ok(main.includes("import CommercialIdentityPortalV2 from './components/CommercialIdentityPortalV2'"), 'Application still mounts the legacy contact portal');
assert.ok(!main.includes("from './components/CommercialIdentityPortal'"), 'Application imports the DOM-scraped legacy portal');
...
assert.ok(existsSync(portalPath), 'React-owned contact portal is missing');
