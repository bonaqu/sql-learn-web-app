...
if (!/(?:rehearsal|restore|staging|test)/i.test(target)) {
  throw new Error('D1_RESTORE_TARGET must visibly identify a non-production rehearsal/staging database');
}
if (confirmation !== 'RESTORE_TO_NON_PRODUCTION') {
  throw new Error('Set ALLOW_D1_RESTORE=RESTORE_TO_NON_PRODUCTION after verifying the target database');
}
...
const match = serialized.match(/"count"\s*:\s*(\d+)/);
if (!match || Number(match[1]) !== 4) throw new Error(`Restore integrity query failed: ${serialized.slice(0, 500)}`);
...
console.log(JSON.stringify({
  ...
  backup,
  requiredTables: 4,
  warning: 'This rehearsal proves importability, not application-level acceptance. Run the full production smoke against the rehearsal Worker next.'
}));
